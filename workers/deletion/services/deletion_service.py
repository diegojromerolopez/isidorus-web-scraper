...
class DeletionService:  # pylint: disable=too-few-public-methods
    ...
    async def __cleanup_s3_objects(self, scraping_id: int):
        """
        ...
        offset = 0
        while True:
            images = await api_models.PageImage.filter(scraping_id=scraping_id).offset(offset).limit(self.__s3_batch_size).values_list("s3_path", flat=True)
            if not images:
                break
    ...
    async def __cleanup_relational_data(self, scraping_id: int):
        """
    ...
    async def __batch_delete(self, model_class: Any, scraping_id: int):
        """
        Helper to delete records in batches.
        """
        while True:
            # Get IDs for the next batch
            ids = await model_class.filter(scraping_id=scraping_id).limit(self.__batch_size).values_list("id", flat=True)
            if not ids:
                break
