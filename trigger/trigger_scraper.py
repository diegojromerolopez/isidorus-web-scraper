...
def submit_job():
    ...
    try:
        ...
        job_id = job_data.get("scraping_id") or job_data.get("id") # Handle potential response variations
...
def monitor_job(job_id):
    ...
    while time.time() - start_time < TIMEOUT:
        try:
            response = requests.get(f"{API_URL}/scrape", params={"scraping_id": job_id}, headers=headers)
            response.raise_for_status()
            data = response.json()
            
            # Assuming the API returns a list or a single object. Adjusting based on likely API structure.
            # If the API returns a list of scrapings, we pick the first one matching our ID.
